...
from PySide2.QtWidgets import QApplication, QMessageBox, QFileDialog, QMainWindow, QDialog, QLabel, QSpacerItem, \
    QSizePolicy
...
class Window(QDialog):
    ...
    def save_single(self):
        """保存生成的单张图片"""
        Directory = QFileDialog.getExistingDirectory(QMainWindow(), "")
        files = list()
        for file in os.listdir(os.path.join(CWD, 'CurOutput') + "\\"):
            files.append(file)
        shutil.copy(os.path.join(CWD, 'CurOutput') + "\\" + files[0], Directory)

    def produce_multiple(self):
        """运行程序，批量生成图片"""
        input_dir = self.ui.InputAddress.text()
        output_dir = self.ui.OutputAddress.text() + os.sep
        if(not os.path.exists(input_dir)):
            QMessageBox.about(self.ui,'提示','批量生成的输入文件夹不存在，请检查路径名')
            return
        if(not os.path.exists(output_dir) or output_dir == os.sep):
...
def generate_image_lock(win, input_dir, output_dir):
    ...
    generate_image(input_dir, output_dir)
...
def make_output_preview():
    Mywindow.ui.OutputPreview.setText("请稍等,效果预览图加载中...")
    shutil.rmtree(os.path.join(CWD, 'CurOutput'))
    os.mkdir(os.path.join(CWD, 'CurOutput'))
    thread1 = Thread(target=draw, args=((os.path.join(CWD,'CurInput'), os.path.join(CWD, 'CurOutput')+"\\")))
    thread1.start()

def draw(input_dir,output_dir):
    generate_image(input_dir,output_dir)
    files = list()
    for file in os.listdir(os.path.join(CWD, 'CurOutput')+"\\"):
        files.append(file)
    Mywindow.ui.OutputPreview.setPixmap(os.path.join(CWD, 'CurOutput')+"\\"+files[0])
